...
namespace JLChnToZ.EditorExtensions.UInspectorPlus {
    ...
    public static class Helper {
        ...
        internal static bool IsInstanceMember(this MemberInfo member, bool defaultResult = false) {
            ...
            if (method != null) return !method.IsStatic && !method.IsConstructor;
        ...
        internal static bool IsTypeRseolvable(Type genericType, Type targetType) {
            if (genericType == null) return true;
            if (!genericType.IsGenericParameter) return genericType == targetType;
            ...
            if ((
                ...
                targetType.GetConstructor(BindingFlags.Instance | BindingFlags.Public, null, CallingConventions.HasThis, EmptyTypes, null) == null
            )) return false;
            foreach (var constrarint in genericType.GetGenericParameterConstraints())
                if (!constrarint.IsAssignableFrom(targetType)) return false;
            return true;
        }
